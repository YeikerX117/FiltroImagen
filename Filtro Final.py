#Filtro laplaciano a imagen "ciclica" sin borde
import cv2
import numpy as np
...
def filtro(imagen, c, s):
    ...
    for i in range (1, 2*(img.shape[0])):
        for j in range(1, 2*(img.shape[1])):
            f=i%img.shape[0]-1
            t=j%img.shape[1]-1
            valor=img.item(f-1,t-1)*c[0][0]+img.item(f-1,t)*c[0][1]+img.item(f-1,t+1)*c[0][2]+img.item(f,t-1)*c[1][0]+img.item(f,t)*c[1][1]+img.item(f,t+1)*c[1][2]+img.item(f+1,t-1)*c[2][0]+img.item(f+1,t)*c[2][1]+img.item(f+1,t+1)*c[2][2]
...
cv2.imwrite("E:\INGENIERIA DE SISTEMAS\Ciencia de datos\Ejercicio Filtro\Imagenes\Prueba1.png", prueba)